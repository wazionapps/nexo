...
Version `7.1.5` is the current packaged-runtime line. It hardens standalone/runtime-root maintenance and release flows: direct catchup runs no longer die just because interactive Claude helpers are absent, explicit-home product checks stop inheriting the operator machine's global Desktop install, and legacy `nexo_doctor` callers keep working without an explicit `plane`. The companion NEXO Desktop client (v0.22.5, closed-source distributed separately) embeds the same coordinated patch line for guided bootstrap, repair, and update flow.
