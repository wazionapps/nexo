...
Version `7.13.4` is the current packaged-runtime line. Corrective release — D.5 correction-learning enforcement is durable, doctor `--fix` explicitly repairs orphan personal schedule metadata, G5 now warns on protected `com.nexo.*.plist` unload/remove flows with the three-layer recovery path, and `installation_live.codex_protocol_compliance` fails live installs whose Codex sessions drift beyond the allowed protocol threshold. Result: coordinated Desktop bundles can ship the fixed Brain without changing the Mac/Windows installation contract.
