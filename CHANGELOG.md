...
- **Automated emails now use the active assistant identity for the sending account.** `nexo-send-reply.py` resolves the display name from runtime calibration/profile first, then account metadata, and finally the sender local-part, so a clean Torrente install no longer signs operator alerts as Nero.
- **Stale autogenerated signatures are replaced safely.** Metadata signatures like `Nero — torrente@nexoagentmail.com` are treated as generated and refreshed to the current identity, while custom signatures remain untouched.
- **Regression coverage pins the identity contract.** Tests cover calibration identity, sender fallback, stale generated signature replacement and custom signature preservation.
...
- **Release and CI tests now stay offline for cognitive search reranking.** Client-parity verification, the general Tests workflow, and the publish workflow force the no-download model mode, and the cognitive reranker respects that flag so release gates cannot hang on HuggingFace downloads.
