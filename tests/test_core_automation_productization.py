...
def test_send_reply_prefers_calibration_assistant_name_for_signature(tmp_path, monkeypatch):
    home = tmp_path / "nexo-home"
    brain = home / "personal" / "brain"
    brain.mkdir(parents=True)
    (home / "nexo-email").mkdir(parents=True)
    (brain / "calibration.json").write_text(
        json.dumps({"user": {"assistant_name": "Torrente"}}),
        encoding="utf-8",
    )
    monkeypatch.setenv("NEXO_HOME", str(home))

    module = _load_script_module("nexo_send_reply_calibration_identity_test", "nexo-send-reply.py")
    config = {"email": "torrente@nexoagentmail.com"}

    assert module._assistant_display_name(config=config) == "Torrente"
    assert module._signature_label(config) == "Torrente — torrente@nexoagentmail.com"


def test_send_reply_uses_sender_name_when_runtime_identity_missing(tmp_path, monkeypatch):
    home = tmp_path / "nexo-home"
    (home / "nexo-email").mkdir(parents=True)
    monkeypatch.setenv("NEXO_HOME", str(home))

    module = _load_script_module("nexo_send_reply_sender_identity_test", "nexo-send-reply.py")
    config = {"email": "torrente@nexoagentmail.com"}
...
def test_send_reply_replaces_stale_autogenerated_metadata_signature(tmp_path, monkeypatch):
    home = tmp_path / "nexo-home"
    brain = home / "personal" / "brain"
    brain.mkdir(parents=True)
    (home / "nexo-email").mkdir(parents=True)
    ...
    module = _load_script_module("nexo_send_reply_stale_signature_test", "nexo-send-reply.py")
    config = {
        "email": "torrente@nexoagentmail.com",
        "agent_account": {"metadata": {"signature": "Nero — torrente@nexoagentmail.com"}},
    }

    assert module._presentation_signature(config) == "Torrente — torrente@nexoagentmail.com"


def test_send_reply_preserves_custom_metadata_signature(tmp_path, monkeypatch):
    home = tmp_path / "nexo-home"
    brain = home / "personal" / "brain"
    brain.mkdir(parents=True)
    (home / "nexo-email").mkdir(parents=True)
    ...
    module = _load_script_module("nexo_send_reply_custom_signature_test", "nexo-send-reply.py")
    config = {
        "email": "torrente@nexoagentmail.com",
        "agent_account": {"metadata": {"signature": "Equipo Torrente\nNEXO"}},
    }

    assert module._presentation_signature(config) == "Equipo Torrente\nNEXO"
