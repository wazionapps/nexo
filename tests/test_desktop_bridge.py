...
def test_onboard_name_step_asks_direct_name():
    import desktop_bridge

    steps = desktop_bridge._onboard_steps()
    name_step = next(step for step in steps if step["id"] == "name")

    assert name_step["prompt"]["es"] == "¿Cómo te llamas?"
    assert name_step["prompt"]["en"] == "What's your name?"
