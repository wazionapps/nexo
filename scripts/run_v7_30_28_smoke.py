#!/usr/bin/env python3
"""Run the curated v7.30.28 release smoke matrix and persist JSON."""
...
DEFAULT_OUTPUT = ROOT / "release-contracts" / "smoke" / "v7.30.28.json"

SMOKE_GROUPS = [
    {
        "id": "f06_core_script_update_propagation",
        "description": "Runtime update copies packaged core scripts through the F0.6 scripts shim and refreshes active version snapshots",
        "targets": [
            ...
            "tests/test_auto_update_f06_shims.py::test_post_install_hook_promotes_f06_core_scripts_from_legacy_conflict",
            "tests/test_auto_update_f06_shims.py::test_post_install_hook_refreshes_core_current_snapshot_even_same_version",
            "tests/test_auto_update_f06_shims.py::test_existing_guardian_hook_runs_f06_runtime_repairs_for_old_hook_lists",
    ...
    },
    {
        ...
        "description": "Release-facing public surfaces are synchronized for v7.30.28",
        "args": [
...
def _command_for_group(group: dict[str, object]) -> list[str]:
    ...
    raise SystemExit(f"[v7.30.28-smoke] group {group.get('id')!r} has no command")
...
def main() -> int:
    ...
    if payload["ok"]:
        print(f"[v7.30.28-smoke] OK -> {output_path}")
        return 0

    failed = [str(group["id"]) for group in groups if not group["ok"]]
    print(f"[v7.30.28-smoke] FAILED groups: {', '.join(failed)}", file=sys.stderr)
    print(f"[v7.30.28-smoke] Summary: {output_path}", file=sys.stderr)
    return 1
