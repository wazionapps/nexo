#!/usr/bin/env python3
"""Run the curated v7.30.23 release smoke matrix and persist JSON."""
...
DEFAULT_OUTPUT = ROOT / "release-contracts" / "smoke" / "v7.30.23.json"
...
SMOKE_GROUPS = [
    ...
    },
    {
        ...
        "description": "Release-facing public surfaces are synchronized for v7.30.23",
        "args": [
...
def _command_for_group(group: dict[str, object]) -> list[str]:
    ...
    raise SystemExit(f"[v7.30.23-smoke] group {group.get('id')!r} has no command")
...
def main() -> int:
    ...
    if payload["ok"]:
        print(f"[v7.30.23-smoke] OK -> {output_path}")
        return 0

    failed = [str(group["id"]) for group in groups if not group["ok"]]
    print(f"[v7.30.23-smoke] FAILED groups: {', '.join(failed)}", file=sys.stderr)
    print(f"[v7.30.23-smoke] Summary: {output_path}", file=sys.stderr)
    return 1
