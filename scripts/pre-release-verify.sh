...
Checks (in order):
  ...
  4. live-source     — scripts/nexo-live-source-preflight.py --domain nexo
  5. managed-mcp-lock — scripts/verify_managed_mcp_lock.py
  6. release-ready   — scripts/verify_release_readiness.py --ci
  7. workflows       — scripts/audit-release-workflows.py (NF-DS-E17D1B61)
  8. pytest          — python3 -m pytest -q
  9. release-target  — free tag + CHANGELOG entry + package.json version
...
run_step live-source    "live source preflight" python3 scripts/nexo-live-source-preflight.py --domain nexo
