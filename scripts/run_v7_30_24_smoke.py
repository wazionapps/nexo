#!/usr/bin/env python3
"""Run the curated v7.30.24 release smoke matrix and persist JSON."""
...
DEFAULT_OUTPUT = ROOT / "release-contracts" / "smoke" / "v7.30.24.json"

SMOKE_GROUPS = [
    {
        "id": "opportunity_orchestrator_contract",
        "description": "Opportunity Orchestrator Phase 1 persists evidence-backed read-only proposals with feedback, suppression and privacy guardrails",
        "targets": [
            "tests/test_opportunity_orchestrator.py",
            "tests/test_migrations.py",
            "tests/test_server_output_schema.py",
        ],
    },
    {
    ...
    },
    {
        ...
        "description": "Release-facing public surfaces are synchronized for v7.30.24",
        "args": [
...
def _command_for_group(group: dict[str, object]) -> list[str]:
    ...
    raise SystemExit(f"[v7.30.24-smoke] group {group.get('id')!r} has no command")
...
def main() -> int:
    ...
    if payload["ok"]:
        print(f"[v7.30.24-smoke] OK -> {output_path}")
        return 0

    failed = [str(group["id"]) for group in groups if not group["ok"]]
    print(f"[v7.30.24-smoke] FAILED groups: {', '.join(failed)}", file=sys.stderr)
    print(f"[v7.30.24-smoke] Summary: {output_path}", file=sys.stderr)
    return 1
