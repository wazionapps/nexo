...
def init_db():
    ...
    _migrate_add_column(conn, "session_diary", "francisco_signals", "TEXT")
...
def write_session_diary(session_id: str, decisions: str, summary: str,
                        ...
                        domain: str = '', francisco_signals: str = '',
                        self_critique: str = '') -> dict:
    """Write a session diary entry with mental state and self-critique for continuity."""
    conn = get_db()
    cleanup_old_diaries()
    cursor = conn.execute(
        "INSERT INTO session_diary (session_id, decisions, discarded, pending, context_next, mental_state, summary, domain, francisco_signals, self_critique) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (session_id, decisions, discarded, pending, context_next, mental_state, summary, domain, francisco_signals, self_critique)
    )
    conn.commit()
    did = cursor.lastrowid
