...
EMAIL_DB_PATH = Path.home() / ".nexo" / "nexo-email" / "nexo-email.db"
EVENT_TABLE_SQL = """
...
def send_email(config, to, cc, subject, body_text, body_html, in_reply_to, references, attachments=None):
    ...
    msg["From"] = formataddr(("NEXO", config["email"]))
    msg["To"] = to
    if cc:
        msg["Cc"] = cc
    msg["Subject"] = subject
    msg["Date"] = formatdate(localtime=True)
    msg["Message-ID"] = make_msgid(domain="systeam.es")
...
def save_to_sent(config, raw_message: bytes, folder: str = "INBOX.Sent"):
...
{html_thread}
        ...
        import html as html_mod
        escaped_body = html_mod.escape(Path(args.body_file).read_text(encoding="utf-8").strip())
...
<p style="color:#888;font-size:11px;">NEXO — nexo@systeam.es</p>
</body></html>"""
