...
def handle_decision_log(domain: str, decision: str, alternatives: str = '',
    ...
    Args:
        domain: Area (frontend, backend, infrastructure, database, api, devops, other)
    ...
    """
    valid_domains = {'frontend', 'backend', 'infrastructure', 'database', 'api', 'devops', 'other'}
...
def handle_decision_search(query: str = '', domain: str = '', days: int = 30) -> str:
    ...
    Args:
        ...
        domain: Filter by area (frontend, backend, infrastructure, database, api, devops, other)
...
def handle_session_diary_write(decisions: str, summary: str,
    ...
    Args:
        ...
        francisco_signals: Observable signals from the user during session — response speed (fast='s' vs detailed explanations), tone (direct, frustrated, exploratory, excited), corrections given, topics he initiated vs topics NEXO initiated. Factual observations only, not interpretations.
        domain: Project context: frontend, backend, infrastructure, database, api, devops, other
    ...
    """
    sid = session_id or 'unknown'
    result = write_session_diary(sid, decisions, summary, discarded, pending, context_next, mental_state, domain=domain, francisco_signals=francisco_signals, self_critique=self_critique)
    if "error" in result:
        return f"ERROR: {result['error']}"
...
def handle_session_diary_read(session_id: str = '', last_n: int = 3, last_day: bool = False,
    ...
    for d in results:
        ...
        if d.get('francisco_signals'):
            lines.append(f"  Señales the user: {d['francisco_signals'][:300]}")
    return "\n".join(lines)
